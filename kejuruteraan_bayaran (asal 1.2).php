...
// ==========================================
// FUNGSI BANTUAN: FORMAT TABLE GRED CIDB (RUJUKAN: kejuruteraan_senarai_kontraktor.php)
// ==========================================
function formatGredTable($gred_str) {
    ...
    if (strpos($gred_raw, "\n") !== false || (strpos($gred_raw, 'GRED') !== false && strpos($gred_raw, 'KATEGORI') !== false)) {
        ...
        $html = '<table class="table table-sm table-bordered m-0 text-center align-middle d-inline-table" style="font-size:0.65rem; width: auto; max-width: 100%; background:#ffffff; border-color:#cbd5e1; margin: 0 auto !important;">';
...
if (isset($_POST['action_sahkan_bayaran'])) {
    ...
    if ($query_kontraktor && $query_kontraktor->num_rows > 0) {
        ...
        $replacements = [
            ...
            '{{ALAMAT}}'   => nl2br(htmlspecialchars($kontraktor['alamat'] ?? $kontraktor['alamat'] ?? 'TIADA MAKLUMAT ALAMAT')),
            '{{PEGAWAI}}'  => strtoupper(htmlspecialchars($kontraktor['nama_pemilik'] ?? $kontraktor['nama_syarikat'] ?? '')),
            '{{GRED}}'     => htmlspecialchars($kontraktor['gred_cidb_kewangan'] ?? '')
        ];
...
// ==========================================
// LOGIK SAHKAN SEMUA BAYARAN (PUKAL / BULK)
// ==========================================
if (isset($_POST['action_sahkan_semua_bayaran'])) {
    $query_semua = $conn->query("SELECT * FROM kontraktor_profil WHERE status_borang = 'Lengkap' AND (status_bayaran_daftar IS NULL OR status_bayaran_daftar != 'Sudah Bayar')");
    ...
    if ($query_semua && $query_semua->num_rows > 0) {
        ...
        echo "<script>alert('Berjaya! Semua bayaran ($jumlah_disahkan kontraktor) telah disahkan dan sijil telah diterbitkan!'); window.location.href='kejuruteraan_bayaran.php';</script>";
        exit();
    } else {
        echo "<script>alert('Tiada bayaran baru yang perlu disahkan.'); window.location.href='kejuruteraan_bayaran.php';</script>";
        exit();
    }
}
            ...
            <div class="container-fluid p-0">
                ...
                <div class="status-card-container mb-4">
                    <div class="d-flex flex-wrap justify-content-between align-items-center border-bottom pb-3 mb-3 gap-2">
                        <h4 class="status-header mb-0"><i class="fa-solid fa-receipt me-2 text-warning"></i>Pengurusan Resit & Sijil Pendaftaran</h4>
                        <form action="" method="POST" class="m-0" onsubmit="return confirm('Adakah anda pasti ingin mengesahkan SEMUA bayaran kontraktor yang belum disahkan?');">
                            <button type="submit" name="action_sahkan_semua_bayaran" class="btn btn-success fw-bold rounded-2 px-3 py-2">
                    ...
                    <div class="table-responsive-custom">
                        <table class="table table-mdbg align-middle">
                            <thead>
                                <tr>
                                    <th class="text-center" style="width: 4%; min-width: 50px;">BIL.</th>
                                    <th style="width: 22%; min-width: 240px;">MAKLUMAT SYARIKAT</th>
                                    <th class="text-center" style="width: 14%; min-width: 180px;">GRED</th>
                                    <th style="width: 20%; min-width: 280px;">1. TINDAKAN BIL</th>
                                    <th style="width: 12%; min-width: 180px;">2. RESIT KONTRAKTOR</th>
                                    <th style="width: 20%; min-width: 280px;">3. TINDAKAN SIJIL</th>
                                    <th style="width: 8%; min-width: 160px;">4. PENGESAHAN</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php 
                                    ...
                                    <td>
                                        <?php if(empty($row['fail_bil'])): ?>
                                            <form action="" method="POST" enctype="multipart/form-data" class="d-flex align-items-center gap-1 m-0">
                                                ...
                                                <input type="file" name="fail_bil" class="form-control form-control-sm" accept="application/pdf" required style="min-width: 200px; max-width: 250px;">
                                    ...
                                    <td>
                                        <?php if(empty($row['fail_sijil'])): ?>
                                            <form action="" method="POST" enctype="multipart/form-data" class="d-flex align-items-center gap-1 m-0">
                                                ...
                                                <input type="file" name="fail_sijil" class="form-control form-control-sm" accept="application/pdf" required style="min-width: 200px; max-width: 250px;" <?= $row['status_bayaran_daftar'] != 'Sudah Bayar' ? 'disabled title="Sila tukar status kontraktor kepada Sudah Bayar terlebih dahulu"' : ''; ?>>
                                    ...
                                    <td>
                                        ...
                                        <?php else: ?>
                                            <form action="" method="POST" class="m-0" onsubmit="return confirm('Adakah anda pasti ingin mengesahkan bayaran bagi kontraktor ini?');">
                                                <input type="hidden" name="profil_id" value="<?= $row['id']; ?>">
                                                <button type="submit" name="action_sahkan_bayaran" class="btn btn-sm btn-success fw-bold text-nowrap rounded-2">
                                ...
                                <tr>
                                    <td colspan="7" class="text-center py-4 text-muted">
                                        ...
                                        Tiada rekod kontraktor berstatus 'Lengkap' ditemui buat masa ini.
                                    </td>
                                </tr>
                                <?php endif; ?>
                            </tbody>
